...
def convert_data(data: List[list]) -> None:
    ...
    for item in data:
        for i in range(len(item)):
            if is_number(item[i]) is True:
                item[i] = float(item[i].strip().lstrip('+'))
...
def is_app_only(station: "Station") -> bool:
    ...
    for i in station:
        if type(i) is str and NO_KIOSK in i:
            return True
    return False
...
def get_station_info(station_id: int, stations: List["Station"]) -> list:
    ...
    for station in stations:
        if station_id == station[ID]:
            # Creates a new list without ID
            info = station[NAME:]
            # LATITUDE, LONGITUDE, and CAPACITY are not needed
            del info[NAME:CAPACITY]
            info.append(not(is_app_only(station)))
    return info
...
def get_column_sum(index: int, stations: List["Station"]) -> int:
    ...
    if index == BIKES_AVAILABLE:
        for station in stations:
            bikes += station[index]
        return bikes

    elif index == DOCKS_AVAILABLE:
        for station in stations:
            docks += station[index]
        return docks
...
def get_stations_with_kiosks(stations: List["Station"]) -> List[int]:
    ...
    for station in stations:
        # Checks if the station has a kiosk
        if is_app_only(station) is False:
            kiosks_stations.append(station[ID])
    return kiosks_stations
...
def get_nearest_station(my_latitude: float, my_longitude: float,
    ...
    for station in stations:
        dist.append(abs(get_lat_lon_distance(my_latitude,
                    my_longitude, station[LATITUDE], station[LONGITUDE])))
    # Picks the latest occuring station if there is a tie
    for i in dist:
        if i <= min(dist):
            near = i
    return stations[dist.index(near)][ID]
...
if __name__ == '__main__':
    ...
    # Campus:',get_nearest_station(43.7868, -79.1896, bike_stations) == 7645)
